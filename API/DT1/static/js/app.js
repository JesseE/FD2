...
var Adata = microAjax("http://dennistel.nl/movies", 
		function(data){
			var movieData = JSON.parse(data);
			for (var i = 0; i<movieData.length; i++) {
				var movies = movieData[i];
				//console.log(movies.title);
				var movieTitle = movies.title;
				var movieDescription = movies.description;
				var movieCover = movies.cover;
				var movieId = movies.id;
				$('.id').append(movieId);
				$('.title').append(movieTitle);
				$('.description').append(movieDescription);
				$('.covers').append(movieCover);
				//console.log(movies);
			}
			//alert(res);	
});
